 ...
 * (POST / -> {status, ...}, GET /media/* -> file bytes) needed to exercise
 * DropZone's full pipeline end-to-end over a real socket, without depending
 * on a real Cobalt deployment or on YouTube itself.
 *
 * `scenario` is a function (payload) => response-shape, letting each test
 * control exactly what the "API" replies with.
 */
export function startFakeCobaltServer({ scenario, mediaFiles }) {
